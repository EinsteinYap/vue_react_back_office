...
export default {
  ...
  getGoodsDetail (goodsId = null) {
    ...
    return new Promise((resolve, reject) => {
      GoodsApi.detail({ goodsId })
        .then(result => {
          this.formData.goods = result.data.detail
          resolve()
        })
    })
  },
  ...
  getFieldsValue () {
    ...
    const goodsFormData = _.pick(goodsInfo, [
      'goods_name', 'images', 'video_id', 'video_cover_id', 'goods_price',
      'stock_total', 'content', 'sales_initial', 'limit_num', 'back_money',
      'total_day', 'status', 'sort', 'isfix',
      'fix_money', 'back_days', 'daily_income',
      'is_spread', 'is_activity', 'language_id',
      'is_bonus',
      'settle_model_label',
      'profit_total',
      'daily_income'
