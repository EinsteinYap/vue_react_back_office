  ...
  >
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        >
          <SelectImage :multiple="false" :default-list="record.logo ? [record.logo] : []" v-decorator="['logo_id']" />
        </a-form-item>
        <a-form-item label="语言名称" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input
            v-decorator="['cname', {rules: [{required: true, min: 2, message: '请输入至少2个字符'}]}]"
          />
        </a-form-item>
        <a-form-item label="语言代号" :labelCol="labelCol" :wrapperCol="wrapperCol" extra="例如: zh_cn, en_us 等">
          <a-input
  ...
  methods: {
    ...
    setFieldsValue () {
      ...
      this.$nextTick(() => {
        ...
        const data = _.pick(this.record, ['cname', 'code', 'sort', 'logo_id'])
        setFieldsValue(data)
      })
    },
    /**
