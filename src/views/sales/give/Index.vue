  ...
  <a-card :bordered="false">
    ...
    >
      <div slot="goods_item" slot-scope="item">
        ...
        <a title="点击查看原图" :href="item.good_image.external_url" target="_blank">
          <img width="70" :src="item.good_image.external_url" alt="产品图片" />
        </a>
<!--        <div>返还本金: <span style="color: red">{{ item.back_money }}</span></div>-->
      </div>
      <div slot="gift_goods_item" slot-scope="item">
        <div v-if="item">
          <div>产品ID: <span style="color: red;">{{ item.goods_id }}</span></div>
          <div>产品名称: <span class="twoline-hide" style="color: red;">{{ item.goods_name }}</span></div>
          <a title="点击查看原图" :href="item.good_image.external_url" target="_blank">
  ...
  data () {
    return {
      ...
      columns: [
        ...
        {
          title: '赠送产品',
          dataIndex: 'gift_goods',
          width: '150px',
          scopedSlots: { customRender: 'gift_goods_item' }
        },
        {
          title: '限购数量',
          dataIndex: 'limit_num'
