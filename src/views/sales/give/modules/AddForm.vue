<template>
  ...
  >
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        <a-form-item label="赠送标题" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input
            v-decorator="['title', {rules: [{required: true, min: 0, message: '请输入赠送标题'}]}]"
          />
        </a-form-item>
        ...
        <a-form-item label="赠送商品" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <SelectGoods
              :multiple="false"
              v-decorator="['gift_goods_id', { rules: [{ required: true, message: '请选择指定的赠送产品' }] }]"
          />
        </a-form-item>
        <a-form-item label="库存数量" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input
            v-decorator="['stock_num', {rules: [{required: true, min: 0, message: '请输入库存数量'}]}]"
          />
        </a-form-item>
        <a-form-item label="限购数量" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input
            v-decorator="['limit_num', {rules: [{required: true, min: 0, message: '请输入限购数量'}]}]"
          />
        </a-form-item>
...
export default {
  ...
  beforeCreate () {
    ...
    this.form.getFieldDecorator('ruleInput', { initialValue: [0], preserve: true })
