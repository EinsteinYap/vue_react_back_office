...
<!--            </a-form-item>-->
            ...
            <a-form-item label="结算模式" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-radio-group
                ...
                v-decorator="['settle_model', {initialValue: 1, rules: [{ required: true }]}]"
              >
            ...
            <a-form-item v-if="changeSettleModelStatus" label="定期总收益" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input-number
                :min="0"
                v-decorator="['profit_total', {initialValue: 0, rules:[{ required: true, message: '请输入定期总收益' }]}]"
              />
            </a-form-item>
            <a-form-item v-if="!changeSettleModelStatus" :label="form.getFieldValue('settle_model') == 4 ? '翻倍基数': '每日收益' " :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input-number
                :step="0.01"
                :min="0"
...
<!--            </a-form-item>-->
            <a-form-item label="到期时间" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-date-picker show-time format="YYYY-MM-DD HH:mm" v-decorator="['end_time']" />
            </a-form-item>
            <a-divider orientation="left">其他设置</a-divider>
            <a-form-item label="语言选择" :labelCol="labelCol" :wrapperCol="wrapperCol" extra="每个产品只能选择单个语言,如需多语言请添加多个商品">
              <a-select v-decorator="['language_id', { initialValue: languageList ? languageList[0].id : -1 , rules: [{required: true, message: '请选择语言'}] }]">
          ...
          <div class="tab-pane" v-show="tabKey == 2">
            <a-form-item label="产品内容" :labelCol="labelCol" :wrapperCol="{span: 16}">
              <Ueditor
                v-decorator="['content', { rules: [{ required: true, message: '产品详情不能为空' }] }]"
              />
            </a-form-item>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    changeSettleModel (el) {
      if (el.target.value === 3) {
        this.changeSettleModelStatus = true
      } else {
        this.changeSettleModelStatus = false
      }
      // 需要更新一下表单
