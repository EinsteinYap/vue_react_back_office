      ...
      <a-form :form="form">
        ...
        <a-form-item label="升级条件" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <InputNumberGroup
            ...
            addonAfter="人"
            :inputProps="{ min: 0 }"
            v-decorator="['upgrade.invite_num', { rules: [{ required: true, message: '升级条件不能为空' }] }]"
          />
          <div class="form-item-help">
            <p class="extra">用户的邀请人数满足后，自动升级</p>
          </div>
        </a-form-item>
        <a-form-item label="下级用户" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <InputNumberGroup
            addonBefore="下级用户消费金额"
            :inputProps="{ min: 0 }"
            v-decorator="['upgrade.child_money', { rules: [{ required: true, message: '升级条件不能为空' }] }]"
          />
          <div class="form-item-help">
            <p class="extra">直推下级每个用户消费金额</p>
          </div>
        </a-form-item>
        <a-form-item label="分红比例" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <InputNumberGroup
            ...
            v-decorator="['bonus', { rules: [{ required: true, message: '分红比例不能为空' }] }]"
          />
          <div class="form-item-help">
            <p class="extra">分红范围0-100，例如: 98%，0代表不分红</p>
          </div>
        </a-form-item>
        <a-form-item label="等级增值" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <InputNumberGroup
            addonAfter="%"
            :inputProps="{ min: 0, max: 100 }"
            v-decorator="['gain', { rules: [{ required: true, message: '等级增值比例不能为空' }] }]"
          />
          <div class="form-item-help">
            <p class="extra">等级增值范围0-100，例如: 98%，0代表不增值</p>
          </div>
        </a-form-item>
...
export default {
  ...
  methods: {
    ...
    setFieldsValue () {
      ...
      this.$nextTick(() => {
        setFieldsValue(pick(record, ['name', 'weight', 'upgrade', 'equity', 'status', 'sort', 'bonus', 'gain']))
      })
    },
