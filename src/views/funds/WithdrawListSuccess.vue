...
<!--          </a-form-item>-->
    ...
    <a-alert banner>
      <template slot="message">
        <div>
          <span style="margin-right: 20px;">已完成金额: <span style="color: red">{{ totalData['2'] }}</span></span>
<!--          <span>累计提现金额: <span style="color: red">{{ totalData['2'] }}</span></span>-->
    ...
    >
      ...
      <div class="actions" slot="action" slot-scope="text, item">
		...
		<a v-if="item.apply_status == 2" @click="handleError(item)">手动失败</a>
	...
	<a-modal v-model="errorVisible" title="设置提现订单失败" ok-text="确认" cancel-text="取消" @ok="okModals">
	  <a-form :form="Shform">
	    <a-form-item label="状态" :labelCol="labelCol" :wrapperCol="wrapperCol">
	      <a-radio-group v-decorator="['apply_status', {initialValue: 1, rules: [{required: true}]}]">
  ...
  methods: {
	...
	okModals() {
      ...
      const modal = this.$confirm({
        title: '你确定手动失败这笔提现订单吗?',
        content: '失败后不可撤回',
        onOk () {
          app.Shform.validateFields((error, values) => {
            if (!error) {
              return Api.error({ id: app.currentItem.id, form: { ...values } })
                .then(result => {
