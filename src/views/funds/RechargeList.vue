  ...
  <a-card :bordered="false">
      ...
      <a-row class="row-item-search">
        <a-form class="search-form" :form="searchForm" layout="inline" @submit="handleSearch">
          ...
          <a-form-item label="充值金额范围">
            <a-input-number v-decorator="['start_money']" placeholder="请输入金额" />
            <span> - </span>
            <a-input-number v-decorator="['end_money']" placeholder="请输入金额" />
          </a-form-item>
          <a-form-item label="支付渠道">
            <a-select v-decorator="['payment_id', { initialValue: -1 }]">
              <a-select-option :value="-1">全部</a-select-option>
              <a-select-option :value="1">现金</a-select-option>
              <a-select-option :value="0">USDT</a-select-option>
            </a-select>
          </a-form-item>
    ...
    <a-alert banner>
      <template slot="message">
        <div>
          ...
          <span style="margin-right: 20px;">累计失败金额: <span style="color: red">{{ totalData.fiail }}</span> </span>
    ...
    >
      ...
      <div slot="user" slot-scope="item">
        <div>所属代理: <span style="color: red">{{ item.user.agent.user_name }}</span></div>
