...
<!--          </a-form-item>-->
          <a-form-item label="订单号">
            <a-input v-decorator="['order_no']" placeholder="请输入订单号搜索" />
          </a-form-item>
          ...
          <a-form-item label="银行卡号">
            <a-input v-decorator="['card_number']" placeholder="请输入银行卡号" />
          </a-form-item>
    ...
    <a-alert banner>
      <template slot="message">
        <div>
          <span style="margin-right: 20px;">支付中金额: <span style="color: red">{{ totalData['1'] }}</span></span>
<!--          <span>累计提现金额: <span style="color: red">{{ totalData['2'] }}</span></span>-->
    ...
    >
      ...
      <div slot="order" slot-scope="item">
        <div v-if="item.user_payment">
          ...
          <div v-if="item.user_payment.cateid == 1">USDT数量: <span style="color: red">{{ item.usdt_num }}</span></div>
          <div v-if="item.user_payment.cateid == 1">转换率: <span style="color: red">{{ item.conver_rate }}%</span></div>
      ...
      <div slot="type" slot-scope="item">
        <div v-if="item.user_payment">
          <div v-if="item.user_payment.mobile">手机号: <span style="color: red">{{ item.user_payment.mobile }}</span></div>
          <div v-if="item.user_payment.ifsc">IFSC: <span style="color: red">{{ item.user_payment.ifsc }}</span></div>
          <div v-if="item.user_payment.cateid == 1">地址: <span style="color: red">{{ item.user_payment.address }}</span></div>
          <div v-if="item.user_payment.cateid == 2">银行卡号: <span style="color: red">{{ item.user_payment.bank_card_number }}</span></div>
          <div>类型: <span style="color: red">{{ item.user_payment.cateid == 1 ? 'USDT' : '银行卡' }}</span></div>
        </div>
        <div v-else> - </div>
      </div>
      <!-- 状态 -->
      ...
      <div slot="operator" slot-scope="item">
      ...
      <div class="actions" slot="action" slot-scope="text, item">
        <a v-if="item.apply_status == 1" @click="handleOperation(item)">手动付款</a>
    ...
    <a-modal v-model="applyVisible" title="审核提现订单" ok-text="确认" cancel-text="取消" @ok="okModal">
      <a-form :form="Shform">
        ...
        <a-form-item v-if="Shform.getFieldValue('apply_status') == 3" label="拒绝原因" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-textarea v-decorator="['reject_reason']" placeholder="请输入拒绝原因" :autoSize="{ minRows: 4, maxRows: 6 }" />
        </a-form-item>
        <a-form-item v-else label="三方支付通道ID" :labelCol="labelCol" :wrapperCol="wrapperCol">
    ...
    <!--    手动付款-->
    <!--    operation-->
    <a-modal v-model="operationVisible" title="手动付款" ok-text="确认" cancel-text="取消" @ok="okModalTwo">
      <a-form :form="Shform">
        <a-form-item label="状态" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-radio-group v-decorator="['apply_status', {initialValue: 2, rules: [{required: true}]}]">
            <a-radio :value="2">付款成功</a-radio>
            <a-radio :value="3">付款失败</a-radio>
          </a-radio-group>
        </a-form-item>
      </a-form>
    </a-modal>
...
import { operation } from '@/api/user/withdraw'
  ...
  methods: {
    ...
    okModalTwo () {
      ...
      const modal = this.$confirm({
        title: '你确定手动设置这笔提现订单吗?',
        content: '审核后不可撤回',
        onOk () {
          app.Shform.validateFields((error, values) => {
            if (!error) {
              return Api.operation({ id: app.currentOperation.id, form: { ...values } })
                .then(result => {
                  app.$message.success(result.message, 1.5)
                  app.currentOperation = {}
                  app.operationVisible = false
