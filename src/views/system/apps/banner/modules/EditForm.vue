    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="广告类型" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-radio-group v-decorator="['typeid', {initialValue: record.typeid, rules: [{required: true}]}]">
            <a-radio :value="1">图片</a-radio>
            <a-radio :value="2">视频</a-radio>
          </a-radio-group>
        </a-form-item>
        <a-form-item v-if="form.getFieldValue('typeid') == 1" label="广告图" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <SelectImage
        ...
        >
          <SelectVideo
            :default-list="record.video?[record.video] : []"
            :multiple="false"
            v-decorator="['video_id',  {rules: [{required: true, message: '请选择1个广告视频'}]}]" />
        </a-form-item>
...
export default {
  components: { SelectVideo, SelectImage },
  props: {
  },
  data () {
    return {
  ...
  methods: {
    ...
    */
    onFormSubmit (values) {
      ...
      Api.edit({ form: newValues, id: this.record.id })
        .then((result) => {
          // 显示成功
