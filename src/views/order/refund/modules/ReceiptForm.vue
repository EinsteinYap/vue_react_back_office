  ...
  >
    <a-spin :spinning="isLoading">
      <a-alert
        ...
        message="请确认已收到寄回的产品，确认后自动退回付款金额（原路退款）并关闭当前售后单"
        banner
      />
      <a-form v-if="visible" :form="form">
        ...
        <a-form-item label="订单付款的总金额" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <span>￥{{ record.orderData.pay_price }}{{ currency_short }}</span>
        </a-form-item>
        <a-form-item label="退款的金额" :labelCol="labelCol" :wrapperCol="wrapperCol">
          ...
          <div class="form-item-help">
            <p
              class="extra"
            >请输入退款金额，最多{{ Math.min(record.orderGoods.total_pay_price, record.orderData.pay_price) }}{{ currency_short }}，最多不能大于订单实际付款的总金额</p>
          </div>
        </a-form-item>
      </a-form>
    </a-spin>
