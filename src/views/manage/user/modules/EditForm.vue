    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="用户昵称" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input
            v-decorator="['nickname', {rules: [{required: true, min: 2, message: '请输入至少2个字符'}]}]"
          />
        </a-form-item>
        <a-form-item label="邮箱地址" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input
            v-decorator="['email', {rules: [{required: true, min: 2, message: '请输入正确的邮箱地址'}]}]"
          />
        </a-form-item>
        <a-form-item label="头像" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <!--          :defaultList="record.avatar_url ? [record.avatar_url] : []"-->
          <SelectImage
            :defaultList=" record.avatar ? [record.avatar] : []"
            v-decorator="['avatar_id', {rules: [{required: true, message: '请选择1个头像'}]}]"
          />
        </a-form-item>
        <a-form-item
...
export default {
  ...
  methods: {
    ...
    setFieldsValue () {
      ...
      this.$nextTick(() => {
        const data = _.pick(this.record, ['user_name', 'real_name', 'sort', 'nickname', 'email', 'status', 'avatar_id'])
