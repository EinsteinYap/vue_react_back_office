    ...
    <a-spin :spinning="isLoading">
      <a-form :form="form">
        ...
        <a-form-item label="昵称" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input
            v-decorator="['nickname', {rules: [{required: true, min: 2, message: '请输入昵称'}]}]"
          />
        </a-form-item>
        <a-form-item label="邮箱地址" :labelCol="labelCol" :wrapperCol="wrapperCol" >
          <a-input
            v-decorator="['email', {rules: [{required: true, min: 2, message: '请输入邮箱地址'}]}]"
          />
        </a-form-item>
        ...
        <a-form-item label="确认密码" :labelCol="labelCol" :wrapperCol="wrapperCol" extra="后台登录密码确认">
          <a-input
            type="password"
    ...
    getInfo () {
      ...
      Api.getInfo()
        .then((result) => {
          ...
          this.form.setFieldsValue(_.pick(userInfo, 'user_name', 'real_name', 'nickname', 'email'))
        })
        .finally(() => {
